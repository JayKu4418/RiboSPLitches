...
def runSNPFold(alreadywrittenfile,mutseqfile, mutfile, baseOfinterest,writefile):
    
    alreadywrittenMuts = pd.read_csv(alreadywrittenfile,sep="\t",header=0)
    
    # Read the file containing sequences around mutation
    mutseqfile = pd.read_csv(mutseqfile,sep="\t",header=None)
    ...
    mutfile = pd.read_csv(mutfile,sep="\t",header=None).iloc[:,range(5,13)].drop_duplicates()
    print mutfile.head()
    ...
    with open(writefile, "w") as fw:
	    ...
	    if mut not in alreadywrittenMuts["MUT"].values:
            ...
            # Call SNP fold and write result into file    
                try:
		    pipe_out = subprocess.check_output(["SNPfold_commandline.py","-m","all","-n",mut+"_"+baseOfinterest,"-A", seq4mut, WTbase + baseOfinterest + MUTbase])
...
# Run function 
runSNPFold("../temp/Muts_Flanking_100bpISREs_"+str(numBasesToFold)+"bpwindow_SNPFoldResults.txt","../temp/Muts_Flanking_100bpISREs_"+str(numBasesToFold)+"bpwindow-Sequences.txt","../temp/Muts_Flanking_100bpISREs_Sorted.txt",str((numBasesToFold/2)+1),"../temp/Muts_Flanking_100bpISREs_"+str(numBasesToFold)+"bpwindow_SNPFoldResults_SecondPart.txt")
